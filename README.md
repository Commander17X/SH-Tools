...
```bash
chmod +x flexfetch/*.sh
```

3. Voeg de tools toe aan je PATH (optioneel):
```bash
echo 'export PATH="$PATH:$HOME/SH-Tools/flexfetch"' >> ~/.bashrc
source ~/.bashrc
```
...
### NetPeek
```bash
./flexfetch/netpeek.sh
```

### LogSift
```bash
./flexfetch/logsift.sh [bestand] [opties]
```

### SnapShot
```bash
./flexfetch/snapshot.sh [--preset] [--compress]
```

### MountMate
```bash
./flexfetch/mountmate.sh [--mount profiel] [--list]
```

### SafePass
```bash
./flexfetch/safepass.sh [--add] [--get] [--export]
```

### LinkFixer
```bash
./flexfetch/linkfixer.sh [bestand] [--github]
```

### TreeDex
```bash
./flexfetch/treedex.sh [pad] [--mode]
```

### CachePurge
```bash
./flexfetch/cachepurge.sh [--dry-run] [--schedule]
```

### BootTrace
```bash
./flexfetch/boottrace.sh [--analyze] [--optimize]
```

### FetchBox
```bash
./flexfetch/fetchbox.sh [url] [--threads] [--resume]
```

### ProMan
```bash
./flexfetch/proman.sh [add|list|done|archive] [project]
```
